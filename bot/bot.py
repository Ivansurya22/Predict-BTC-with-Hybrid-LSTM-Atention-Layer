...
ADMIN_USER_ID = int(os.getenv("ADMIN_USER_ID", "0"))  # Tambahkan di .env file

# File untuk menyimpan data user yang sudah subscribe
USERS_DATA_FILE = "bot/users_data.json"
USER_FILTER_MODES_FILE = "bot/user_filter_modes.json"
...
FILTER_MODES = {
    'safe': {
        'name': 'Safe Mode 🛡️',
        'description': 'Untuk trader yang mengutamakan keamanan',
        'min_confidence': 0.80,
        'min_supporting_factors': 3,
        'max_conflicting_factors': 0,
        'emoji': '🛡️',
        'characteristics': 'Sinyal berkualitas tinggi, jarang tapi akurat'
    },
    'balanced': {
        'name': 'Balanced Mode ⚖️',
        'description': 'Untuk trader yang ingin keseimbangan risk-reward',
        'min_confidence': 0.60,
        'min_supporting_factors': 2,
        'max_conflicting_factors': 1,
        'emoji': '⚖️',
        'characteristics': 'Sinyal moderat, frekuensi sedang dengan akurasi baik'
    },
    'aggressive': {
        'name': 'Aggressive Mode ⚡',
        'description': 'Untuk trader yang ingin lebih banyak peluang',
        'min_confidence': 0.40,
        'min_supporting_factors': 1,
        'max_conflicting_factors': 2,
        'emoji': '⚡',
        'characteristics': 'Sinyal lebih sering, risiko lebih tinggi tapi peluang lebih banyak'
    }
}

# Global variable untuk menyimpan mode filter setiap user
user_filter_modes = {}
...
def add_user_to_subscribers(user_id, chat_id):
    ...
    users_data[str(user_id)] = {
        ...
        'active': True,
        'last_alert_sent': None  # Track alert terakhir untuk user ini
    }
...
# Fungsi untuk load/save user filter modes
def load_user_filter_modes():
    try:
        with open(USER_FILTER_MODES_FILE, 'r') as f:
            return json.load(f)
    except (FileNotFoundError, json.JSONDecodeError):
        return {}

def save_user_filter_modes(modes_data):
    with open(USER_FILTER_MODES_FILE, 'w') as f:
        json.dump(modes_data, f, indent=2)

# Fungsi untuk set user filter mode
def set_user_filter_mode(user_id, mode):
    global user_filter_modes
    user_filter_modes = load_user_filter_modes()
    user_filter_modes[str(user_id)] = mode
    save_user_filter_modes(user_filter_modes)

# Fungsi untuk get user filter mode
def get_user_filter_mode(user_id):
    global user_filter_modes
    user_filter_modes = load_user_filter_modes()
    return user_filter_modes.get(str(user_id), 'balanced')  # Default balanced

# Fungsi untuk cek apakah prediksi memenuhi filter mode
def check_filter_criteria(prediction_data, mode):
    """Cek apakah prediksi memenuhi kriteria filter mode yang dipilih"""
    ...
    # Ambil kriteria mode
    filter_config = FILTER_MODES.get(mode, FILTER_MODES['balanced'])
    
    # Cek confidence model (bukan SMC confidence)
    if model['confidence'] < filter_config['min_confidence']:
        return False
    
    # Cek SMC supporting dan conflicting factors jika tersedia
    if smc:
        supporting_count = len(smc.get('supporting_factors', []))
        conflicting_count = len(smc.get('conflicting_factors', []))
        
        if supporting_count < filter_config['min_supporting_factors']:
            return False
        
        if conflicting_count > filter_config['max_conflicting_factors']:
            return False
...
async def send_welcome_alert(chat_id, user_id):
    ...
    try:
        ...
        if current_price or prediction_data:
            # Buat pesan welcome dengan data terbaru
            ...
            if prediction_data:
                ...
                message_parts.append(f"📈 Buy: {probs['buy']*100:.0f}% | Sell: {probs['sell']*100:.0f}% | Hold: {probs['hold']*100:.0f}%")
            ...
            # Update waktu alert terakhir untuk user ini
...
async def start(update: Update, context: CallbackContext):
    ...
    if is_admin(user_id):
        keyboard = [
            ["📊 Cek Harga BTC", "🎯 Filter Alert"],
            ["🔮 Hasil Prediksi", "📊 Hasil Backtest"],
            ["⚙️ Pengaturan Alert", "ℹ️ Status Subscription"],
    ...
    # Kirim alert data terbaru secara asinkron
...
async def admin_stats(update: Update, context: CallbackContext):
    ...
    # Hitung user yang bergabung hari ini
...
async def enable_alert(update: Update, context: CallbackContext):
    ...
    if str(user_id) in users_data:
        ...
        # Kirim data terbaru setelah mengaktifkan alert
        asyncio.create_task(send_welcome_alert(chat_id, user_id))
    else:
...
# Fungsi untuk force alert (mendapatkan data terbaru sekarang)
...
async def btc_price(update: Update, context: CallbackContext):
    ...
    # Tambahan informasi waktu update
    current_time = datetime.now().strftime("%d %b %Y, %H:%M")
...
async def filter_alert(update: Update, context: CallbackContext):
    ...
    # Buat keyboard untuk pilihan mode
    from telegram import InlineKeyboardButton, InlineKeyboardMarkup
    
    keyboard = []
    for mode_key, mode_config in FILTER_MODES.items():
        # Tambahkan tanda centang untuk mode yang aktif
    ...
    # Tambahkan tombol info dan test filter
    keyboard.append([
...
async def handle_filter_callback(update: Update, context: CallbackContext):
    ...
    if data.startswith("filter_mode_"):
        ...
        # Test filter dengan data terbaru
        current_mode = get_user_filter_mode(user_id)
        prediction_data = get_latest_prediction()
        ...
        if prediction_data:
            ...
            message = (
                ...
                f"• Supporting Factors: {len(smc.get('supporting_factors', []))}\n"
                f"• Conflicting Factors: {len(smc.get('conflicting_factors', []))}\n\n"
...
async def btc_prediction(update: Update, context: CallbackContext):
    # Dapatkan data prediksi dari database
    prediction_data = get_latest_prediction()
    ...
    if prediction_data:
        ...
        # Buat pesan dengan format yang bagus
        ...
        message += f"• Buy: {model['probabilities']['buy']*100:.1f}%\n"
        message += f"• Sell: {model['probabilities']['sell']*100:.1f}%\n"
        message += f"• Hold: {model['probabilities']['hold']*100:.1f}%\n\n"
        
        # Tambahkan evaluasi SMC jika tersedia
        if smc:
            smc_confidence_pct = f"{smc['confidence']:.1f}%"
            setup_type = smc['setup_type']
            
            message += f"<b>Konfirmasi SMC:</b> {setup_type} (Confidence: {smc_confidence_pct})\n\n"
            
            # Tampilkan detail faktor pendukung
            if smc['supporting_factors']:
                message += "<b>Faktor Pendukung:</b>\n"
                for factor in smc['supporting_factors']:
                    message += f"✅ {factor}\n"
                message += "\n"
            
            # Tampilkan detail faktor bertentangan
            if smc['conflicting_factors']:
                message += "<b>Faktor Bertentangan:</b>\n"
                for factor in smc['conflicting_factors']:
                    message += f"❌ {factor}\n"
        else:
            message += "<b>Evaluasi SMC:</b> Tidak tersedia"
...
# Fungsi untuk menampilkan hasil backtest (Coming Soon)
async def btc_backtest(update: Update, context: CallbackContext):
    await update.message.reply_text("📊 Hasil Backtest: Coming Soon...")

# Fungsi untuk membuat pesan alert otomatis
def create_filtered_alert_message(prediction_data, current_price, user_id, is_periodic=False):
    """Buat pesan alert yang sudah difilter sesuai preferensi user"""
    ...
    # Tambahkan info filter mode user
    user_mode = get_user_filter_mode(user_id)
    mode_config = FILTER_MODES[user_mode]
    
    message_parts.append(f"🚨 <b>{alert_type}</b> 🚨")
    message_parts.append(f"📅 {current_time}")
    message_parts.append(f"🎯 Filter: {mode_config['emoji']} {mode_config['name'].split()[0]}\n")
    ...
    # Informasi prediksi jika ada yang baru DAN lolos filter
    if prediction_data:
        model = prediction_data['model']
        timestamp = pd.to_datetime(model['timestamp'])
        
        # Cek apakah ini data baru
        if (not last_sent_data['prediction_timestamp'] or 
            timestamp > pd.to_datetime(last_sent_data['prediction_timestamp'])):
            
            # Cek apakah lolos filter user
            if check_filter_criteria(prediction_data, user_mode):
                signal_emoji = {
                    'buy': '🔼', 
                    'sell': '🔽',
                    'hold': '↔️'
                ...
                message_parts.append(f"🔮 <b>Prediksi Filtered:</b> {signal_emoji} {model['signal'].upper()}")
                message_parts.append(f"📊 <b>Confidence:</b> {confidence_pct}")
                
                # Tambahkan info SMC jika tersedia
                smc = prediction_data.get('smc', {})
                if smc:
                    supporting_count = len(smc.get('supporting_factors', []))
                    conflicting_count = len(smc.get('conflicting_factors', []))
                ...
                # Tambahkan probabilitas singkat
                probs = model['probabilities']
                message_parts.append(f"📈 Buy: {probs['buy']*100:.0f}% | Sell: {probs['sell']*100:.0f}% | Hold: {probs['hold']*100:.0f}%")
    ...
    if is_periodic and len(message_parts) > 3:  # 3 karena ada header + filter info
        return "\n".join(message_parts)
...
async def send_filtered_alert_to_subscribers(message_base, prediction_data, current_price, alert_type="regular"):
    global bot_app
    ...
    # Kirim ke semua user yang aktif dengan filter masing-masing
    active_users = [
        (user_id, user_data) 
    ...
    for user_id, user_data in active_users:
        try:
            # Buat pesan yang sudah difilter untuk user ini
            filtered_message = create_filtered_alert_message(
                prediction_data, 
                current_price, 
                int(user_id), 
            ...
            print(f"Error sending filtered message to {user_data['chat_id']}: {e}")
    
    print(f"✅ {alert_type.title()} filtered alert sent to {sent_count} users")
    return sent_count

# Fungsi untuk mengirim broadcast message admin
async def send_broadcast_message(message):
    sent_count = await send_filtered_alert_to_subscribers()(message, "admin broadcast")
    return sent_count
...
async def send_hourly_alert():
    try:
        ...
        # Buat pesan alert berkala
        alert_message = create_filtered_alert_message()(prediction_data, current_price, is_periodic=True)
        
        if alert_message:
            await send_filtered_alert_to_subscribers()(alert_message, "hourly")
        else:
...
# Thread function untuk menjalankan alert berkala
def hourly_alert_scheduler():
    """Fungsi untuk menjalankan alert setiap jam di background thread"""
    global bot_app
    
    print("🔔 Hourly alert scheduler started...")
    ...
    while True:
        try:
            ...
            if bot_app and bot_app.running:
                # Gunakan event loop dari bot yang sedang running
                try:
                    # Schedule coroutine pada event loop yang sedang running
                    asyncio.run_coroutine_threadsafe(
                        send_hourly_alert(),
                        bot_app.updater.loop if hasattr(bot_app, 'updater') else asyncio.get_event_loop()
                    ).result(timeout=30)
                except Exception as e:
                    print(f"❌ Error scheduling hourly alert: {e}")
                
        except Exception as e:
            print(f"❌ Error in hourly alert scheduler: {e}")
            time.sleep(60)  # Tunggu 1 menit sebelum coba lagi

# Thread function untuk monitoring real-time
def realtime_monitor_scheduler():
    """Fungsi untuk monitoring data baru secara real-time"""
    global bot_app
        ...
        try:
            if bot_app and bot_app.running:
                ...
                try:
                    ...
                    print(f"❌ Error scheduling real-time monitor: {e}")
                    time.sleep(60)
                
        except Exception as e:
            print(f"❌ Error in real-time monitor scheduler: {e}")
            time.sleep(60)  # Tunggu 1 menit sebelum coba lagi
...
async def schedule_background_tasks():
    ...
    # Create tasks untuk background monitoring
...
# Lanjutan dari handle_admin_broadcast_message
async def handle_admin_broadcast_message(update: Update, context: CallbackContext):
    ...
    try:
        ...
        if update.message.photo:
            ...
            sent_count = await send_broadcast_message(broadcast_message)
            await update.message.reply_text(
...
async def send_broadcast_photo(photo_file_id, caption):
    ...
    # Kirim ke semua user yang aktif
...
async def handle_message(update: Update, context: CallbackContext):
    ...
    # Cek apakah admin dalam mode broadcast
    if user_id in admin_broadcast_mode:
    ...
    elif text == "🎯 Filter Alert":
        await filter_alert(update, context)
    elif text == "🔮 Hasil Prediksi":
        await btc_prediction(update, context)
    elif text == "📊 Hasil Backtest":
        await btc_backtest(update, context)
...
async def help_command(update: Update, context: CallbackContext):
    help_text = (
        ...
        "💰 Harga BTC terkini\n"
        "🔮 Prediksi trading (Buy/Sell/Hold)\n"
        "📈 Analisis teknikal\n"
        "📊 Confidence score\n\n"
...
async def error_handler(update: object, context: CallbackContext):
    ...
    # Kirim error ke admin jika dalam mode development
    if ADMIN_USER_ID and hasattr(update, 'effective_chat'):
        try:
            ...
            pass  # Gagal kirim ke admin, abaikan
...
def start_bot():
    ...
    bot_app.add_handler(CallbackQueryHandler(handle_filter_callback))
    ...
    # OPTION 1: Gunakan background tasks (Recommended)
    print("🔔 Starting background alert services...")
    
    # Tambahkan post_init untuk menjalankan background tasks
    async def post_init(application):
        # Start background tasks setelah bot siap
        asyncio.create_task(schedule_background_tasks())
    
    bot_app.post_init = post_init
    
    # OPTION 2: Gunakan threading (Alternative, comment jika menggunakan Option 1)
    # hourly_thread = threading.Thread(target=hourly_alert_scheduler, daemon=True)
    # hourly_thread.start()
    # realtime_thread = threading.Thread(target=realtime_monitor_scheduler, daemon=True)
...
# Fungsi untuk menghentikan bot dengan graceful shutdown - FIXED
def stop_bot():
