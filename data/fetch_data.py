...
class BTCDataFetcher:
    def __init__(self):
        # Gunakan exchange PUBLIC (tanpa API key untuk data OHLCV)
    ...
    def _format_candles(self, candles):
        ...
        return [
            (
                ...
                datetime.fromtimestamp(c[0] / 1000, tz=timezone.utc).strftime("%Y-%m-%d %H:%M:%S%z"),
                c[1], c[2], c[3], c[4], c[5]
            )
            for c in candles
        ]
    ...
    def fetch_historical(self, symbol="BTC/USDT", force_from_2020=False):
        ...
        if force_from_2020:
            ...
            last_date = datetime.fromtimestamp(since / 1000, tz=timezone.utc).strftime('%Y-%m-%d %H:%M')
        ...
        while since < current_time:
            try:
                ...
                latest_time = datetime.fromtimestamp(closed[-1][0] / 1000, tz=timezone.utc)
                print(f"✓ Progress: {latest_time.strftime('%Y-%m-%d %H:%M')} | Total: {total_candles} candles")
                ...
                print(f"⚠ Error (attempt {consecutive_errors}/{max_consecutive_errors}): {e}")
    ...
    async def realtime_monitor(self, symbol="BTC/USDT"):
        ...
        while True:
            try:
                ...
                if ohlcv:
                    current = ohlcv[-1]
                    current_time_str = datetime.fromtimestamp(current[0] / 1000, tz=timezone.utc).strftime("%Y-%m-%d %H:%M:%S")
                    print(f"📊 Current candle (in progress): {current_time_str}")
                    print(f"   O: ${current[1]:,.2f} | H: ${current[2]:,.2f} | L: ${current[3]:,.2f} | C: ${current[4]:,.2f}")
                ...
                print(f"⏳ Next check at {next_update.strftime('%H:%M:%S')} UTC ({wait_time:.0f}s)\n")
    ...
    def run(self):
        ...
        if not self.test_connection():
            print("\n✗ Cannot connect to Binance. Please check your internet connection.")
            return
