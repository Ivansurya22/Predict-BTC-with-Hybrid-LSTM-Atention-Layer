...
class BTCPredictor:
    """
    Bitcoin Price Predictor - Aligned with Balanced Preprocessing
    ...
    def __init__(self, model_path, scaler_path, feature_cols_path, sequence_length_path,
        ...
        print(f"\n{'='*70}")
        print(f"🔧 Initializing BTC Predictor")
        print(f"{'='*70}")
    ...
    def fetch_latest_data(self):
        ...
        # Handle NaN values (SAME AS PREPROCESSING - FIXED deprecation)
    ...
    def preprocess_data(self, df):
        ...
        ALIGNED: Same as preprocessing pipeline
        """
        print(f"\n{'='*70}")
...
def compare_predictions(db_path='data/btc_ohlcv.db', hours_back=24):
    ...
    for _, row in df_pred.iterrows():
        ...
        if len(df_actual) > 0:
            ...
            # Determine actual outcome (aligned with preprocessing thresholds)
            if price_change > 1.0:
                actual_outcome = 'UP'
