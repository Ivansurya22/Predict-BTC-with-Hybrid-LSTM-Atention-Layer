...
class ModelManagementTab(QWidget):
    ...
    def create_prediction_tab(self):
        ...
        self.prediction_log.setPlainText(
            ...
            "Requirements:\n"
            "• Trained model must exist (models/lstm_full.pth)\n"
    ...
    def create_progress_section(self):
        ...
        # Status log (DIPERBESAR)
        self.status_log = QTextEdit()
        self.status_log.setReadOnly(True)
        self.status_log.setMinimumHeight(450)  # Diperbesar dari 300
...
"""
    ...
    def run_prediction(self):
        ...
        if not model_path.exists():
            QMessageBox.warning(
                self,
                "Model Not Found",
                f"<b>Trained model not found!</b><br><br>"
                f"Expected location:<br>"
                f"<code>{model_path}</code><br><br>"
                f"Please train the model first from the Training tab."
            )
            return
    ...
    def show_prediction_result(self, result):
        ...
        QMessageBox.information(
            ...
            f"<b>Current Price:</b> ${result['close_price']:,.2f}"
        )
    ...
    def view_prediction_history(self):
        ...
        if not db_path.exists():
            QMessageBox.warning(
                self,
                "Database Not Found",
                "Database not found. No prediction history available."
            )
            return
        ...
        try:
            ...
            if not cursor.fetchone():
                QMessageBox.information(
                    self,
                    "No History",
                    "No prediction history found. Run predictions first."
                )
                conn.close()
                return
            ...
            history_window.setWindowTitle("📜 Prediction History")
            history_window.resize(1000, 600)

            layout = QVBoxLayout(history_window)

            # Summary
            summary = QLabel(
                f"<b>Total Predictions:</b> {len(df)}<br>"
                f"<b>Latest:</b> {df['timestamp'].iloc[0]}<br>"
                f"<b>Oldest:</b> {df['timestamp'].iloc[-1]}"
            )
            summary.setStyleSheet("padding: 10px; font-size: 12px;")
            ...
            for i, row in df.iterrows():
                table.setItem(i, 0, QTableWidgetItem(str(row['timestamp'])))
                table.setItem(i, 1, QTableWidgetItem(row['prediction']))
                table.setItem(i, 2, QTableWidgetItem(f"{row['confidence']:.1%}"))
                table.setItem(i, 3, QTableWidgetItem(f"{row['prob_down']:.1%}"))
                table.setItem(i, 4, QTableWidgetItem(f"{row['prob_hold']:.1%}"))
                table.setItem(i, 5, QTableWidgetItem(f"{row['prob_up']:.1%}"))
                table.setItem(i, 6, QTableWidgetItem(f"${row['close_price']:,.2f}"))
            ...
            QMessageBox.critical(
                self,
                "Error",
                f"Failed to load prediction history:\n\n{str(e)}"
            )
